...
sys.path.insert(0, os.path.join(os.path.dirname(__file__), 
    'build/lib.macosx-14.0-arm64-cpython-314'))

from neug import Database, Connection
...
def test_louvain_with_properties():
    ...
    conn.execute("MATCH (a:person {id: 1}), (b:person {id: 2}) CREATE (a)-[:knows {weight: 1.0}]->(b);")
    conn.execute("MATCH (a:person {id: 1}), (b:person {id: 3}) CREATE (a)-[:knows {weight: 1.0}]->(b);")
    conn.execute("MATCH (a:person {id: 2}), (b:person {id: 4}) CREATE (a)-[:knows {weight: 1.0}]->(b);")
    conn.execute("MATCH (a:person {id: 3}), (b:person {id: 5}) CREATE (a)-[:knows {weight: 1.0}]->(b);")
    conn.execute("MATCH (a:person {id: 4}), (b:person {id: 5}) CREATE (a)-[:knows {weight: 1.0}]->(b);")
