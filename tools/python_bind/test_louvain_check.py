...
sys.path.insert(0, os.path.join(os.path.dirname(__file__), 'build/lib.macosx-14.0-arm64-cpython-314'))
...
for pid, nm, ag in [(0,"Alice",30),(1,"Bob",25),(2,"Charlie",35),(3,"Dave",28),(4,"Eve",22),(5,"Frank",40),(6,"Grace",33),(7,"Hank",29)]:
    conn.execute(f'CREATE (p:Person {{id: {pid}, name: "{nm}", age: {ag}}});')

for i in range(4):
    for j in range(i+1,4):
        conn.execute(f'MATCH (a:Person {{id: {i}}}), (b:Person {{id: {j}}}) CREATE (a)-[:Knows]->(b);')
        conn.execute(f'MATCH (a:Person {{id: {i}}}), (b:Person {{id: {j}}}) CREATE (b)-[:Knows]->(a);')
...
print(f"共 {len(rows)} 行, 每行 {len(rows[0])} 列")
