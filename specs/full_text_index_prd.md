...
全文索引用于在大量文本中快速检索某些关键词，一般采用倒排索引的方式实现。
...
倒排索引以“词语”为中心，当用户查询“手机”时，系统可以直接通过关键词定位到相关文档列表，而不需要扫描全部文档。理想情况下，关键词在词典中的定位可以接近 `O(1)`。
...
全文索引常用于检索和某个关键词相关的属性，并通过各种指标计算关联度，如 BM25。

例如以下查询可以检索description中的 `Alice`，并按照关联度从高到低展示：
```
MATCH (n: Person)
ORDER BY fts_bm25(n.description, 'Alice')
RETURN n
LIMIT 5;
```

注意：全文索引并不能完全替代Contains。例如 `'Alice' Contains 'Ali'`，但是全文索引中 `Ali` 会被当作一个单独的单词检索，并不会找到 `Alice`。这一点在中文中更加明显：例如 `我喜欢吃苹果` 会被切分为 `我`、`喜欢`、`吃`、`西瓜`，此时直接检索 `瓜` 是无法匹配的。
...
考虑到 NeuG 需要负责事务管理，因此直接将索引数据存在内部表中是最合理的实现方案，这样才能确保索引和数据具有相同的事务隔离型（DuckDB和Sqlite都是这么实现的）。事实上，为了确保事务隔离的一致性，索引数据都应该尽量存放在内部表中（HNSW无法做到，属于特例）。

另一方面，我们也可以尽量吸收各种产品中的存储优化和查询优化，来逐渐提升全文索引的效率。
